        ...
        <br>
            ...
            <div class="list-liquor">
                <div>
                    ...
                    <div class="item">
                        <ul>
                            <a class = "detail" href = "{{ action('Admin\LiquorController@detail', ['id' => $beer->id]) }}">
                                <li><img src="{{ $beer->image_path }}"></li>
                                <li class="index_name">{{ $beer->name }}</li>
                                <li>{{ $beer->comment }}</li>
                            </a>
                            <li>
                                <div>
                                    @if($rogin_id === $beer->id || $rogin_id ===1)
                                        <a class="edit" href="{{ action('Admin\LiquorController@edit', ['id' => $beer->id]) }}">編集</a>
                                        <a class="delete" href="{{ action('Admin\LiquorController@delete', ['id' => $beer->id]) }}">削除</a>
                                    @else
                                        <a class="edit">編集</a>
                                        <a class="delete">削除</a>
                                    @endif
